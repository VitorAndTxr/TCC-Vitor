...
A \texttt{BluetoothScreen} (Figura~\ref{fig:irisMobileTelaPareamento}) é o ponto de entrada do fluxo de coleta e ilustra o padrão de organização visual do aplicativo: cabeçalho com o título da seção e ação contextual de atualização da lista, agrupamento ``\textsc{Paired Devices}'' contendo os dispositivos previamente pareados ao sistema operacional --- cada um representado por um cartão com nome, identificador físico (\emph{MAC address}) ou intensidade de sinal e etiqueta de estado (\emph{Available} ou \emph{Connected}) ---, e barra de navegação global persistente no rodapé (\emph{Home}, \emph{History}, \emph{Bluetooth}, \emph{Settings}), por meio da qual o usuário transita entre as demais telas do aplicativo.
