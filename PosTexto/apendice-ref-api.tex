\chapter{Referência da API REST do Nó de Pesquisa Interoperável}\label{cap:apendiceRefApi}
...
\begin{tabular}{|p{0.13\textwidth}|p{0.84\textwidth}|}
\hline
...
\texttt{GET /api/channel/\newline\{channelId\}} & Consulta os metadados de um canal aberto (uso diagnóstico). & \texttt{A} \\ \hline
...
\texttt{POST /api/node/\newline authenticate} & Fase~3: submete o desafio assinado com a chave privada RSA-2048 e recebe o \textit{token} de sessão de aplicação. & \texttt{C} \\ \hline
\end{longtable}
...
\begin{longtable}{|p{0.35\textwidth}|p{0.45\textwidth}|p{0.15\textwidth}|}
...
\texttt{GET /api/node/\newline getactiveconnectionspaginated} & Lista, paginadamente, as conexões aprovadas e ativas. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/node/\newline getallunaprovedpaginated} & Lista, paginadamente, as conexões pendentes de aprovação. & \texttt{C+S+U} \\ \hline
\texttt{POST /api/node/\newline \{connectionId\}/approve} & Aprova uma solicitação de conexão pendente. & \texttt{C+S+U} \\ \hline
\texttt{POST /api/node/\newline \{connectionId\}/reject} & Rejeita uma solicitação de conexão pendente. & \texttt{C+S+U} \\ \hline
...
\texttt{POST /api/userauth/\newline refreshtoken} & Renova o \textit{token} JWT antes da expiração, sem nova apresentação de credencial. & \texttt{C+S+U} \\ \hline
...
\texttt{GET /api/researcher/\newline getresearchers} & Lista pesquisadores paginados. & \texttt{C+S+U} \\ \hline
...
\texttt{GET /api/researcher/\newline getresearcherbynodeid/\{nodeId\}} & Lista os pesquisadores associados a um nó remoto. & \texttt{C+S+U} \\ \hline
\texttt{PUT /api/researcher/update/\newline \{id:guid\}} & Atualiza dados de um pesquisador. & \texttt{C+S+U} \\ \hline
\end{longtable}
...
\texttt{GET /api/research/\newline getallpaginatedasync} & Lista projetos paginados. & \texttt{C+S+U} \\ \hline
...
\texttt{GET /api/research/\newline\{researchId:guid\}/researchers} & Lista pesquisadores vinculados a um projeto. & \texttt{C+S+U} \\ \hline
\texttt{POST /api/research/\newline\{researchId:guid\}/researchers} & Vincula um pesquisador a um projeto. & \texttt{C+S+U} \\ \hline
\texttt{PUT /api/research/\newline\{researchId:guid\}/researchers/\newline\{researcherId:guid\}} & Atualiza o vínculo de um pesquisador (papel, datas). & \texttt{C+S+U} \\ \hline
\texttt{DELETE /api/research/\newline\{researchId:guid\}/researchers/\newline\{researcherId:guid\}} & Remove o vínculo de um pesquisador. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/research/\newline\{researchId:guid\}/volunteers} & Lista voluntários inscritos em um projeto. & \texttt{C+S+U} \\ \hline
\texttt{POST /api/research/\newline\{researchId:guid\}/volunteers} & Inscreve um voluntário em um projeto. & \texttt{C+S+U} \\ \hline
\texttt{PUT /api/research/\newline\{researchId:guid\}/volunteers/\newline\{volunteerId:guid\}} & Atualiza a inscrição de um voluntário. & \texttt{C+S+U} \\ \hline
\texttt{DELETE /api/research/\newline\{researchId:guid\}/volunteers/\newline\{volunteerId:guid\}} & Remove a inscrição de um voluntário. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/research/\newline\{researchId:guid\}/applications} & Lista aplicações cliente autorizadas a operar sobre um projeto. & \texttt{C+S+U} \\ \hline
\texttt{POST /api/research/\newline\{researchId:guid\}/applications} & Autoriza uma aplicação cliente a operar sobre um projeto. & \texttt{C+S+U} \\ \hline
\texttt{PUT /api/research/\newline\{researchId:guid\}/applications/\newline\{applicationId:guid\}} & Atualiza a autorização de uma aplicação. & \texttt{C+S+U} \\ \hline
\texttt{DELETE /api/research/\newline\{researchId:guid\}/applications/\newline\{applicationId:guid\}} & Revoga a autorização de uma aplicação. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/research/\newline\{researchId:guid\}/devices} & Lista dispositivos vinculados a um projeto. & \texttt{C+S+U} \\ \hline
\texttt{POST /api/research/\newline\{researchId:guid\}/devices} & Vincula um dispositivo a um projeto. & \texttt{C+S+U} \\ \hline
\texttt{PUT /api/research/\newline\{researchId:guid\}/devices/\newline\{deviceId:guid\}} & Atualiza o vínculo de um dispositivo. & \texttt{C+S+U} \\ \hline
\texttt{DELETE /api/research/\newline\{researchId:guid\}/devices/\newline\{deviceId:guid\}} & Remove o vínculo de um dispositivo. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/research/\newline\{researchId:guid\}/sensors} & Lista todos os sensores associados a um projeto. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/research/\newline\{researchId:guid\}/devices/\newline\{deviceId:guid\}/sensors} & Lista os sensores de um dispositivo específico no escopo de um projeto. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/research/\newline\{id:guid\}/export} & Exporta o acervo completo de um projeto em arquivo \textit{ZIP}. & \texttt{C+S+U} \\ \hline
\end{longtable}
...
\caption*{\ABNTEXfontereduzida{\bfseries\nomefonte: Autoria própria (2026).}} \\
\endlastfoot
\texttt{GET /api/volunteer/\newline getallpaginated} & Lista voluntários paginados. & \texttt{C+S+U} \\ \hline
\texttt{POST /api/volunteer/new} & Cadastra um novo voluntário. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/volunteer/\newline \{id:guid\}} & Recupera um voluntário por identificador. & \texttt{C+S+U} \\ \hline
\texttt{PUT /api/volunteer/\newline update/\{id:guid\}} & Atualiza dados de um voluntário. & \texttt{C+S+U} \\ \hline
\texttt{DELETE /api/volunteer/\newline \{id:guid\}} & Remove um voluntário. & \texttt{C+S+U} \\ \hline
\end{longtable}
...
\texttt{GET /api/clinicalsession/\newline getallpaginated} & Lista sessões clínicas paginadas, com filtros opcionais por pesquisa, voluntário, \textit{status} e janela temporal. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/clinicalsession/\newline \{id:guid\}} & Recupera uma sessão clínica por identificador. & \texttt{C+S+U} \\ \hline
\texttt{PUT /api/clinicalsession/\newline update/\{id:guid\}} & Atualiza atributos administrativos de uma sessão clínica. & \texttt{C+S+U} \\ \hline
\texttt{POST /api/clinicalsession/\newline \{sessionId:guid\}/recordings/new} & Registra uma nova gravação associada a uma sessão clínica. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/clinicalsession/\newline \{sessionId:guid\}/recordings} & Lista as gravações de uma sessão clínica. & \texttt{C+S+U} \\ \hline
\texttt{POST /api/clinicalsession/\newline \{sessionId:guid\}/annotations/new} & Adiciona uma anotação temporal a uma sessão clínica. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/clinicalsession/\newline \{sessionId:guid\}/annotations} & Lista as anotações de uma sessão clínica. & \texttt{C+S+U} \\ \hline
...
Este grupo concentra o CRUD das entidades de catálogo da subárea de vocabulário clínico (Seção~\ref{sec:apendiceBancoVocabulario}), organizadas em quatro categorias: estruturas anatômicas (\texttt{BodyStructure}), regiões corporais (\texttt{BodyRegion}), modificadores topográficos (\texttt{TopographicalModifier}) e condições clínicas (\texttt{ClinicalCondition}). A chave natural é, em todos os casos, o código SNOMED.
...
\caption*{\ABNTEXfontereduzida{\bfseries\nomefonte: Autoria própria (2026).}} \\
\endlastfoot
\texttt{GET /api/snomed/BodyStructure/\newline getactivebodystructures} & Lista estruturas anatômicas ativas. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/snomed/BodyStructure/\newline getallbodystructurespaginatedasync} & Lista estruturas anatômicas paginadas. & \texttt{C+S+U} \\ \hline
\texttt{POST /api/snomed/BodyStructure/new} & Cadastra uma nova estrutura anatômica. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/snomed/BodyStructure/\newline \{snomedCode\}} & Recupera uma estrutura anatômica pelo código SNOMED. & \texttt{C} \\ \hline
\texttt{PUT /api/snomed/BodyStructure/\newline update/\{snomedCode\}} & Atualiza uma estrutura anatômica. & \texttt{C} \\ \hline
\texttt{GET /api/snomed/BodyRegion/\newline getactivebodyregions} & Lista regiões corporais ativas. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/snomed/BodyRegion/\newline getallbodyregionspaginatedasync} & Lista regiões corporais paginadas. & \texttt{C+S+U} \\ \hline
\texttt{POST /api/snomed/BodyRegion/new} & Cadastra uma nova região corporal. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/snomed/BodyRegion/\newline \{snomedCode\}} & Recupera uma região corporal pelo código SNOMED. & \texttt{C} \\ \hline
\texttt{PUT /api/snomed/BodyRegion/\newline update/\{snomedCode\}} & Atualiza uma região corporal. & \texttt{C} \\ \hline
\texttt{GET /api/snomed/\newline TopographicalModifier/\newline getactivetopographicalmodifiers} & Lista modificadores topográficos ativos. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/snomed/\newline TopographicalModifier/\newline getalltopographicalmodifierspaginatedasync} & Lista modificadores topográficos paginados. & \texttt{C+S+U} \\ \hline
\texttt{POST /api/snomed/\newline TopographicalModifier/new} & Cadastra um novo modificador topográfico. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/snomed/\newline TopographicalModifier/\{snomedCode\}} & Recupera um modificador topográfico pelo código SNOMED. & \texttt{C} \\ \hline
\texttt{PUT /api/snomed/\newline TopographicalModifier/update/\newline \{snomedCode\}} & Atualiza um modificador topográfico. & \texttt{C} \\ \hline
\texttt{GET /api/snomed/ClinicalCondition/\newline getactiveclinicalconditions} & Lista condições clínicas ativas. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/snomed/ClinicalCondition/\newline getallclinicalconditionspaginated} & Lista condições clínicas paginadas. & \texttt{C+S+U} \\ \hline
\texttt{POST /api/snomed/ClinicalCondition/\newline new} & Cadastra uma nova condição clínica. & \texttt{C+S+U} \\ \hline
\texttt{GET /api/snomed/ClinicalCondition/\newline \{snomedCode\}} & Recupera uma condição clínica pelo código SNOMED. & \texttt{C} \\ \hline
\texttt{PUT /api/snomed/ClinicalCondition/\newline update/\{snomedCode\}} & Atualiza uma condição clínica. & \texttt{C} \\ \hline
\end{longtable}
...
\begin{longtable}{|p{0.35\textwidth}|p{0.40\textwidth}|p{0.15\textwidth}|}
...
\texttt{GET /api/sync/snomed/\newline \{entityType\}} & Exporta entidades de catálogo SNOMED CT (estruturas, regiões, modificadores, condições) com paginação. & \texttt{C+S(R/W)} \\ \hline
...
\texttt{GET /api/sync/recordings/\newline \{id:guid\}/file} & Recupera o conteúdo binário de um canal de gravação por identificador, em envelope JSON cifrado. & \texttt{C+S(R/W)} \\ \hline
