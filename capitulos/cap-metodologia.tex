...
As etapas de desenvolvimento do Padrão de Rótulos e Interfaces para Sistemas Médicos (PRISM) foram inicialmente definidas de acordo com o demonstrado pela Figura \ref{fig:metodologiaGeral}. Primeiramente, é realizada a revisão da literatura sobre o assunto, na qual são identificados os requisitos e as tecnologias esperados de uma solução como a apresentada neste trabalho. Em seguida, na etapa de escopo da proposta, o autor delimita os processos que descrevem como os componentes participantes do sistema irão interagir entre si e quais as estruturas de dados obrigatórias para essa interação.
...
Definido o escopo, prossegue-se para a construção e implantação dos artefatos, iniciando-se pelo Nó de Pesquisa Interoperável (NPI), que servirá como meio de interoperabilidade e espinha dorsal da proposta. Finalizada a construção do NPI, é desenvolvido um projeto exemplo, constituído por um sistema de gestão de acesso e voluntários integrado a um sistema de captura de sEMG orquestrado pelo PRISM, no qual o armazenamento dos dados produzidos pela solução e a gestão de acesso às informações são realizados pelo NPI. Com o projeto exemplo pronto para uso, parte-se para a etapa de coleta de dados dos voluntários, populando o sistema com os dados e biossinais adquiridos. Por fim, são validados os resultados obtidos.
...
O NeuroEstimulator é um dispositivo de eletroestimulação funcional(\textit{Functional Electrical Stimulation} --- FES) que emprega como gatilho a ativação muscular detectada por seu próprio sensor de sEMG de um canal. O microcontrolador adotado é o ESP32, responsável tanto pela orquestração da aquisição do sinal quanto pela comunicação com sistemas externos.
...
O dispositivo utilizado neste trabalho é uma evolução do equipamento proposto por \citeonline{veroFES}, tendo sido implementado pelo presente autor em coautoria com colegas no âmbito da disciplina \textit{Oficinas de Integração 3} do curso de Engenharia de Computação da UTFPR. O módulo AD8232 incorpora as modificações propostas por \citeonline{jjj8232}.

Para os fins deste trabalho, foram aproveitados exclusivamente os módulos relacionados à captura do sinal de sEMG, sendo descartadas as funcionalidades de eletroestimulação, uma vez que o foco da pesquisa
...
Com o objetivo de gerar dados para testar a funcionalidade de sincronização entre dois NPIs distintos, adotou-se como protocolo a contração isométrica do bíceps, com posterior criação das etiquetas correspondentes na plataforma.

Os procedimentos de aquisição foram aprovados pelo Comitê de Ética em Pesquisa com Seres Humanos da Universidade Tecnológica Federal do Paraná (CEP/UTFPR), sob parecer nº~7.218.395.

Neste trabalho, os sinais foram coletados do próprio autor, uma vez que o cerne da investigação reside na estrutura dos dados e em sua sincronização entre nós, e não na qualidade do sinal capturado. Complementarmente, o Capítulo~\ref{cap:resultados} apresenta uma análise dos dados obtidos quando a entrada do sistema é alimentada por um gerador de funções com parâmetros conhecidos, permitindo avaliar quantitativamente a fidelidade da cadeia de aquisição.
