...
complementares no Capítulo~\ref{cap:resultados}: a
...
final da integração foi conduzido na Seção
...
validação aos aspectos lógicos da sincronização entre nós, como o
...
banda fisiológica do sEMG (cerca de 500\,Hz)\cite{deluca1997use}.
...
experimento descrito no Capítulo~\ref{cap:desenvolvimento}, com quatro
...
A quarta e última limitação diz respeito à \textbf{cobertura de
...
Por fim, a imagem do NPI é construída com
...
A primeira é a semântica de \textit{last-write-wins} sobre o
...
nós e não preserva a causalidade das modificações..

A segunda é a ausência de \textit{checksum} explícito por
...
A terceira é a sobrecarga de codificação. O \textit{payload}
...
A quarta é a descoberta manual entre nós. O operador do
...
distribuída. Essa decisão é deliberada, pois dados clínicos
...
dispositivos distintos. Cenários multicanal emergiriam naturalmente dessa
reformulação.
...
clínicos sobre o IRIS e o IRIS \textit{Mobile} permitiria trazer funcionalidades e estrutura mais condizentes com a rotina de perquisa.
