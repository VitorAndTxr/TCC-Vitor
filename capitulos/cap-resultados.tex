...
Este capítulo apresenta a validação experimental do conjunto de artefatos descritos no Capítulo~\ref{cap:desenvolvimento}. A hipótese central deste trabalho --- de que dois Nós de Pesquisa Interoperáveis (NPIs) autônomos são capazes de reconciliar seus acervos preservando a semântica clínica e a soberania local sobre os dados --- é exercitada por meio da execução fim a fim do protocolo de sincronização proposto pelo PRISM \textit{Middleware}. No cenário avaliado, um nó produtor (NPI-A) cria registros de pesquisa e um nó consumidor (NPI-B) solicita uma cópia desses registros via \textit{pull}, demonstrando que o contrato detalhado na Subseção~\ref{subsec:NPITrocaDados} é exequível em condições reais e que os artefatos exportados preservam a semântica clínica do nó produtor.
...
A Seção~\ref{sec:resultadosSincCenario} detalha o cenário experimental e o procedimento executado; a Seção~\ref{sec:resultadosSincExport} examina a estrutura dos artefatos efetivamente exportados pelo NPI-B; e a Seção~\ref{sec:resultadosSincIdempotencia} verifica as propriedades de idempotência e de \textit{last-write-wins} declaradas no contrato de troca de dados entre nós.
...
% ======================================================================
\section{Cenário e procedimento}\label{sec:resultadosSincCenario}
% ======================================================================
...
\begin{enumerate}
    \item Handshake completo entre NPI-B e NPI-A, percorrendo
    ...
    \item População do NPI-A com os dados produzidos para
    ...
    \item Pré-visualização da sincronização, por meio da
    ...
    \item \textit{Pull} efetivo (\texttt{POST /api/sync/pull}),
...
\hline
\textbf{Subárea} & \textbf{Entidades transferidas} & \textbf{Formato} & \textbf{Origem na API} \\ \hline
...
A Listagem~\ref{codigo:resultadosSincPayloadJson} apresenta o \textit{payload} JSON de uma \texttt{RecordingSession} importada pelo NPI-B, no qual é possível verificar a preservação dos códigos SNOMED~CT que identificam a região anatômica do voluntário --- no exemplo, o bíceps (código \texttt{16225001}) com modificador de lateralidade direita (código \texttt{24028007}) --- bem como das anotações textuais associadas à sessão.
...
Na sequência, a Listagem~\ref{codigo:resultadosSincRecordJson} exibe o \textit{payload} JSON do \texttt{Record} associado à \texttt{RecordingSession} exemplificada, contendo a referência ao \textit{blob} de cada \texttt{RecordChannel} hospedado no \textit{storage} do NPI-B, a taxa de amostragem efetiva (215\,Hz) e o número total de amostras transferidas.
...
Já a Listagem~\ref{codigo:resultadosSincCsvCabecalho} reproduz o cabeçalho e as primeiras linhas do arquivo CSV correspondente a um \texttt{RecordChannel} importado, no qual cada linha registra o instante de amostragem em milissegundos e o valor numérico bruto produzido pelo conversor analógico-digital do NeuroEstimulator.
...
Por fim, a Figura~\ref{fig:resultadossEMGPlot} apresenta a representação gráfica do mesmo arquivo CSV, na qual se observa, ao longo do tempo, o sinal sEMG capturado do bíceps direito do voluntário e importado pelo NPI-B ao término da sincronização.

\begin{figure}[htb]
\caption{Registro do sinal sEMG capturado do bíceps.}
\label{fig:resultadossEMGPlot}
\includegraphics[width=1\textwidth]{figuras/Resultados/record_signal_plot.png}
\fonte{Autoria própria (2026)}
\end{figure}
...
% ======================================================================
\section{Verificação de idempotência e \textit{last-write-wins}}\label{sec:resultadosSincIdempotencia}
% ======================================================================
...
1ª & 17 & 0 & Primeira execução, esperado o conjunto completo \\ \hline
...
A semântica de \textit{last-write-wins} sobre o atributo \texttt{UpdatedAt} foi observada em um experimento complementar no qual a mesma \texttt{SnomedBodyRegion} foi editada em ambos os nós em uma janela curta de tempo. O comportamento observado confirma a estratégia descrita na Subseção~\ref{subsec:NPITrocaDados}, segundo a qual o nó cuja escrita detém o maior \texttt{UpdatedAt} prevalece. Reconhece-se, contudo, que essa semântica é vulnerável ao desvio de relógio entre nós e não preserva a causalidade das modificações --- limitação retomada na Subseção~\ref{subsec:limitacoesProtocolo} e contemplada como trabalho futuro na Subseção~\ref{subsec:trabalhosFuturosPrism}.
...
\ref{sec:resultadosCadeiaAquisicao}, \ref{sec:resultadosSincExport}
e \ref{sec:resultadosSincIdempotencia}.
...
\textbf{NPI} & Viabilidade da camada de \textit{middleware} de interoperabilidade entre nós. & Seções \ref{sec:resultadosSincExport} e \ref{sec:resultadosSincIdempotencia} & \textit{Handshake} concluído, sincronização incremental e idempotência demonstradas (Quadros \ref{quad:resultadosSincExecucao} e \ref{quad:resultadosSincIdempotencia}). \\ \hline
\textbf{IRIS} & Integração de aplicações generalistas ao padrão sem acoplamento ao tipo de biossinal. & Seção \ref{sec:resultadosSincCenario} & Operação completa do fluxo de pré-visualização e \textit{pull} via interface \textit{web}, sem dependência de processamento de sEMG. \\ \hline
\textbf{IRIS Mobile} & Consumo do mesmo contrato do NPI por clientes em plataformas heterogêneas. & Seção \ref{sec:resultadosSincCenario} & Captura do gerador de funções e envio ao NPI-A pelo aplicativo Android, materializada nos arquivos sincronizados (Codigos~\ref{codigo:resultadosSincPayloadJson} e \ref{codigo:resultadosSincCsvCabecalho}). \\ \hline
